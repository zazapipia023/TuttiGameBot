...
public class DeleteVkGameAction implements Action {
    ...
    private void deleteGame(Long chatId, String game) {
        if (DefaultGames.vkGames.contains(game)) {
            ...
            if (club.getVkGames().contains(game)) {
                List<String> games = club.getVkGames();
                games.remove(game);
