...
public class DeleteBattleNetGameAction implements Action {
    ...
    private void deleteGame(Long chatId, String game) {
        if (DefaultGames.battleNetGames.contains(game)) {
            ...
            if (club.getBattleNetGames().contains(game)) {
                List<String> games = club.getBattleNetGames();
                games.remove(game);
