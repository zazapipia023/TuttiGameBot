...
public class DeleteEgsGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"delete_egs_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("delete_egs_game");
        clientService.save(client);
        log.info("Saved action \"delete_egs_game\" to user: " + chatId);
    }
}
