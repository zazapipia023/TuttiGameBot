...
public class DeleteVkGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"delete_vk_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("delete_vk_game");
        clientService.save(client);
        log.info("Saved action \"delete_vk_game\" to user: " + chatId);
    }
}
