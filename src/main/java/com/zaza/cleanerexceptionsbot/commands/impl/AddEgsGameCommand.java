...
public class AddEgsGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"add_egs_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("add_egs_game");
        clientService.save(client);
        log.info("Saved action \"add_egs_game\" to user: " + chatId);
    }
}
