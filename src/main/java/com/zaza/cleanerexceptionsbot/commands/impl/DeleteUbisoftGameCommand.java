...
public class DeleteUbisoftGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"delete_ubisoft_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("delete_ubisoft_game");
        clientService.save(client);
        log.info("Saved action \"delete_ubisoft_game\" to user: " + chatId);
    }
}
