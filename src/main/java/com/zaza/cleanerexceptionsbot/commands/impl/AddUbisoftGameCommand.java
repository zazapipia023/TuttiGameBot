...
public class AddUbisoftGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"add_ubisoft_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("add_ubisoft_game");
        clientService.save(client);
        log.info("Saved action \"add_ubisoft_game\" to user: " + chatId);
    }
}
