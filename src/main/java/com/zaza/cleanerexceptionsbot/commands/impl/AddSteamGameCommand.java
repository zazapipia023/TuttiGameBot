...
public class AddSteamGameCommand implements Command<Long> {
    ...
    private void sendMessage(Long chatId) {
        String message = "Напишите название папки с игрой и ID игры в формате \"Название игры:ID\" \n" +
                "ID игры можно узнать на странице игры в Steam";
        telegramSender.sendMessage(chatId, message);
    }

    private void setAction(Long chatId) {
        log.info("Setting action \"add_steam_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("add_steam_game");
        clientService.save(client);
        log.info("Saved action \"add_steam_game\" to user: " + chatId);
    }
}
