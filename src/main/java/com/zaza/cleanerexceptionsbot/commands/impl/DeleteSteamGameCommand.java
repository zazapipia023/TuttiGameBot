...
public class DeleteSteamGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"delete_steam_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("delete_steam_game");
        clientService.save(client);
        log.info("Saved action \"delete_steam_game\" to user: " + chatId);
    }
}
