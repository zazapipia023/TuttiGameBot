...
public class AddVkGameCommand implements Command<Long> {
    ...
    private void setAction(Long chatId) {
        log.info("Setting action \"add_vk_game\" to user: " + chatId);
        TelegramClient client = clientService.findOne(chatId);
        client.setAction("add_vk_game");
        clientService.save(client);
        log.info("Saved action \"add_vk_game\" to user: " + chatId);
    }
}
